= newpipe-plugin
:toc: left
:toc-title: Contents
:toclevels: 3
:icons: font
:source-highlighter: highlight.js
:hardbreaks-option:
:plugin_version: 0.0.1

++++
<p align="right">
  <a href="README_fr.adoc">
    <img src="images/lang-fr-red.svg" alt="Français" width="64"/>
  </a>
</p>
++++

image:https://img.shields.io/badge/Kotlin-2.3.20-7F52FF?logo=kotlin[Kotlin]
image:https://img.shields.io/badge/Gradle-9.5.1-02303A?logo=gradle[Gradle]
image:https://img.shields.io/badge/Plugin_Portal-cccp--education-blue[Plugin Portal]
image:https://img.shields.io/badge/License-Apache%202.0-blue.svg[License]

A Gradle plugin (`education.cccp.newpipe`) that downloads music from YouTube
and converts it to MP3 — no Python dependency, pure JVM.

== Value Proposition

In a JVM ecosystem, integrating `yt-dlp` requires Python, `pip`, and Python
version management across machines and CI agents. This plugin eliminates that
friction: a single Gradle dependency, zero mandatory system tools beyond
FFmpeg (or Docker).

== Architecture

=== Overview

[source,text]
----
newpipe-plugin/
└── newpipe/
    └── src/
        ├── main/kotlin/com/cheroliv/newpipe/
        │   ├── DownloaderPlugin.kt      Plugin entry point
        │   ├── NewpipeExtension.kt      DSL extension (configPath, ffmpegDockerImage, forceDocker, sessionsPath)
        │   ├── NewpipeManager.kt        Task registration + YAML parsing
        │   ├── Models.kt                Selection / Artist / Session data classes
        │   ├── DownloadMusicTask.kt     Main Gradle task
        │   ├── VideoServices.kt         VideoInfoProvider / AudioConverter interfaces + VideoMetadata
        │   ├── YouTubeDownloader.kt     Real implementation (NewPipe + OkHttp)
        │   ├── Mp3Converter.kt          Real implementation (FFmpeg / Docker + jaudiotagger)
        │   ├── DownloaderImpl.kt        NewPipe Downloader (OkHttp singleton)
        │   ├── AuthSessionTask.kt       OAuth2 Device Flow authentication
        │   ├── BuildSessionsTask.kt     Session construction from client_secrets
        │   ├── SessionManager.kt        Round-Robin session distribution + refresh
        │   ├── TokenRefresher.kt        Silent OAuth2 token refresh
        │   ├── AuthErrorHandler.kt      Auth error mapping + structured logging
        │   ├── AgeVerificationHandler.kt Age-restricted video detection
        │   ├── PrivatePlaylistHandler.kt Private playlist access
        │   ├── SessionStatusTask.kt     Session monitoring dashboard
        │   ├── SessionMonitor.kt        Session usage tracking
        │   └── InteractiveCliTask.kt    Interactive CLI menu
        ├── test/kotlin/com/cheroliv/newpipe/
        │   ├── FakeVideoInfoProvider.kt Network-free fake for tests
        │   ├── FakeAudioConverter.kt    FFmpeg-free fake for tests
        │   ├── AgeVerificationHandlerTest.kt
        │   ├── AuthErrorHandlerTest.kt
        │   ├── NewpipePluginTest.kt
        │   ├── PrivatePlaylistHandlerTest.kt
        │   └── SessionManagerTest.kt
        ├── test/scenarios/com/cheroliv/newpipe/scenarios/
        │   ├── AuthSessionSteps.kt      Cucumber steps
        │   ├── CucumberTestRunner.kt
        │   ├── MinimalSteps.kt
        │   └── NewpipeWorld.kt
        └── functionalTest/kotlin/com/cheroliv/newpipe/
            ├── tools.kt
            ├── YouTubeAuthIntegrationTest.kt
            └── YouTubeIntegrationTest.kt
----

=== Execution Flow

[source,text]
----
musics.yml
    │
    ▼
NewpipeManager.registerDownloadTask()
    │  parse YAML → Selection → tuneEntries + playlistUrls
    ▼
DownloadMusicTask.download()
    │
    ├── Playlist resolution (sequential)
    │     └── VideoInfoProvider.getPlaylistVideoUrls()
    │           ├── Mix Radio (list=RD...) → first page only (~15 tracks)
    │           └── Standard playlist     → all pages (cap at 50)
    │
    └── Concurrent pipeline (Semaphore 3) for each URL
          │
          ├── fetchAudio()
          │     ├── VideoInfoProvider.getVideoInfo()               → VideoMetadata
          │     ├── VideoInfoProvider.getBestAvailableBitrateKbps() → int (kbps)
          │     ├── AudioConverter.alreadyDownloaded()             → skip or upgrade
          │     │     └── quality upgrade → rename .mp3 → .old (backup)
          │     └── VideoInfoProvider.downloadBestAudio()          → _temp.m4a
          │           └── stream selected: 320k → 256k → 192k → 160k → fallback
          │
          └── convertTrack() — immediately after download completes
                ├── AudioConverter.convertToMp3()                  → FFmpeg <bitrate>k
                ├── AudioConverter.addMetadata()                   → ID3 tags + cover
                │     └── full pass, then minimal fallback (title+artist) if needed
                └── on success → delete .old backup
                    on failure → restore .old as .mp3
----

== Core Components

=== VideoInfoProvider / AudioConverter

Two interfaces allow real implementations to be swapped for fakes in tests
without crossing the GradleTestKit JVM boundary.

[source,kotlin]
----
interface VideoInfoProvider {
    suspend fun getVideoInfo(url: String): VideoMetadata
    suspend fun getPlaylistVideoUrls(playlistUrl: String): List<String>
    suspend fun downloadBestAudio(
        metadata: VideoMetadata,
        outputFile: File,
        onProgress: (downloaded: Long, total: Long, percent: Int) -> Unit
    ): File
    fun sanitizeFileName(name: String): String

    // Returns the bitrate (kbps) of the best stream available for this metadata.
    // Returns 0 in fake/test mode — disables the quality upgrade check.
    fun getBestAvailableBitrateKbps(metadata: VideoMetadata): Int
}

interface AudioConverter {
    // bestAvailableBitrateKbps = 0 disables the quality upgrade check (fake/test mode).
    fun alreadyDownloaded(
        mp3File: File,
        title: String,
        artist: String,
        youtubeDurationSeconds: Long,
        toleranceSeconds: Long = 2L,
        bestAvailableBitrateKbps: Int = 0
    ): Boolean
    suspend fun convertToMp3(inputFile: File, outputFile: File, bitrate: String = "192k"): File
    suspend fun addMetadata(
        mp3File: File,
        title: String?,
        artist: String?,
        album: String?,
        thumbnailUrl: String?
    ): File
}
----

=== VideoMetadata

A clean data class that decouples the plugin from NewPipe Extractor's
`StreamInfo` (a `final` class that cannot be instantiated from outside the
library).

[source,kotlin]
----
data class VideoMetadata(
    val title: String,
    val uploaderName: String,
    val duration: Long,
    val url: String,
    val streamInfo: StreamInfo? = null  // null in test mode
)
----

=== NewpipeExtension — DSL

The DSL extension exposes three properties to the consumer's `build.gradle.kts`:

[source,kotlin]
----
open class NewpipeExtension @Inject constructor(objects: ObjectFactory) {

    // Required — absolute path to musics.yml
    val configPath: Property<String> = objects.property(String::class.java)

    // Optional — Docker image for FFmpeg conversion. Default: "jrottenberg/ffmpeg:4.4-alpine"
    val ffmpegDockerImage: Property<String> = objects
        .property(String::class.java)
        .convention(DEFAULT_FFMPEG_IMAGE)

    // Optional — force Docker even if FFmpeg is installed locally. Default: false
    val forceDocker: Property<Boolean> = objects
        .property(Boolean::class.java)
        .convention(false)
}
----

[cols="1,2,1,2", options="header"]
|===
| Property | Type | Default | Description

| `configPath`
| `String`
| —
| *Required.* Absolute path to the `musics.yml` selection file.

| `ffmpegDockerImage`
| `String`
| `jrottenberg/ffmpeg:4.4-alpine`
| Docker image used for conversion when FFmpeg is not installed locally,
or when `forceDocker = true`.

| `forceDocker`
| `Boolean`
| `false`
| When `true`, skips the local FFmpeg probe and always uses Docker,
even if `ffmpeg` is available on the system `PATH`.
|===

=== Mp3Converter — FFmpeg Strategy

The conversion strategy is resolved *once at construction*, in priority order:

[cols="1,1,3", options="header"]
|===
| Priority | Strategy | Condition

| 1
| `LOCAL`
| `ffmpeg -version` exits 0 *and* `forceDocker = false`

| 2
| `DOCKER`
| `docker info` exits 0 (daemon running)

| 3
| `NONE`
| Neither available → `ConversionException` at conversion time
|===

When the `DOCKER` strategy is active, the plugin runs:

[source,bash]
----
docker run --rm \
  --user <current-uid>:<current-gid> \
  -v <artist-dir>:/data \
  <ffmpegDockerImage> \
  -i /data/<input.m4a> -vn -ar 44100 -ac 2 -b:a <bitrate>k -y \
  /data/<output.mp3>
----

The `--user` flag ensures the MP3 is owned by the user running Gradle (not the
container's root), preventing permission errors when jaudiotagger writes the
ID3 tags. Both the temp file and the output MP3 share the same artist directory,
so a single volume mount covers both.

The `<bitrate>` value is the actual bitrate of the selected YouTube stream —
not a fixed constant — ensuring the conversion faithfully preserves the
source quality.

=== Mp3Converter — Duplicate Detection and Quality Upgrade

`alreadyDownloaded()` checks three criteria before deciding to skip a track:

[cols="1,3", options="header"]
|===
| Criterion | Detail

| ID3 tags
| `TITLE` and `ARTIST` tags must match the YouTube metadata exactly

| Duration
| Encoded duration must be within ±2 seconds of the YouTube video duration

| Bitrate
| Encoded bitrate must be within 32 kbps of the best available YouTube stream.
A gap > 32 kbps triggers a quality upgrade re-download.
|===

The 32 kbps threshold absorbs normal FFmpeg encoding variance (a 320 kbps
stream typically produces 315–318 kbps encoded). Only a meaningful quality
gap triggers a re-download.

==== Tag writing — two-pass strategy

`addMetadata()` writes ID3 tags in two passes to avoid losing a file on error:

. *Full pass* — title, artist, album, year, cover art. Throws on any error.
. *Minimal fallback* — title and artist only, if the full pass threw.
. If both passes fail, the MP3 is kept without tags rather than failing the
download entirely — a warning is logged.

=== Mp3Converter — Quality Upgrade Safety

When `alreadyDownloaded()` detects a quality upgrade opportunity, the existing
MP3 is not deleted immediately. Instead, `DownloadMusicTask` manages a safe
three-phase process:

[source,text]
----
1. Rename <n>.mp3 → <n>.old   (backup before download starts)
2. Download new stream + convert to MP3
3. Success → delete <n>.old
   Failure → restore <n>.old as <n>.mp3
----

At no point is the user left without an audio file for a given track.

=== YouTubeDownloader — Audio Stream Selection

`downloadBestAudio()` selects the best audio stream using a descending bitrate
ladder. Format (m4a, webm, opus…) is intentionally ignored — FFmpeg converts
regardless of source format.

[cols="1,1,2", options="header"]
|===
| Priority | Target | Selection rule

| 1
| 320 kbps
| Lowest stream with `averageBitrate >= 320`

| 2
| 256 kbps
| Lowest stream with `averageBitrate >= 256`

| 3
| 192 kbps
| Lowest stream with `averageBitrate >= 192`

| 4
| 160 kbps
| Lowest stream with `averageBitrate >= 160`

| Fallback
| Best available
| Highest bitrate stream — warning logged
|===

"Lowest stream >= target" avoids overkill: a 256 kbps stream is preferred over
a 320 kbps stream when the target is 256 kbps.

`getBestAvailableBitrateKbps()` reuses the same selection logic without
downloading, so `alreadyDownloaded()` can perform the quality comparison before
any network I/O.

=== DownloadMusicTask — Concurrency

Downloads are concurrent (up to 3 at a time via `Semaphore`). FFmpeg
conversion starts immediately after each download in the same coroutine —
`_temp.m4a` files do not accumulate.

[source,kotlin]
----
val semaphore = Semaphore(MAX_CONCURRENT_DOWNLOADS) // = 3

allEntries.map { (artistHint, tuneUrl) ->
    async(Dispatchers.IO) {
        semaphore.withPermit {
            val track = fetchAudio(tuneUrl, artistHint, baseOutputDir, label)
            convertTrack(converter, track)  // immediate conversion
        }
    }
}.awaitAll()
----

The `DownloadedTrack` data class carries `selectedBitrateKbps` (the bitrate of
the chosen YouTube stream) and `previousMp3Backup` (the `.old` file reference
during a quality upgrade), keeping all state local to the pipeline without
shared mutable state.

=== DownloaderImpl — OkHttp Singleton

A single `OkHttpClient` is shared between metadata extraction (NewPipe
Extractor) and audio download — avoiding redundant TCP/TLS handshakes and
thread pool multiplication.

[source,kotlin]
----
companion object {
    val httpClient: OkHttpClient = OkHttpClient.Builder()
        .connectTimeout(30, TimeUnit.SECONDS)
        .readTimeout(60, TimeUnit.SECONDS)
        .writeTimeout(30, TimeUnit.SECONDS)
        .build()
}
----

The download buffer is 128 KB to minimise kernel syscall overhead.

== Testing Strategy

=== The Problem

GradleTestKit runs the plugin in a separate JVM — Mockito cannot cross that
boundary. Real YouTube downloads + FFmpeg make Cucumber tests slow (2+ minutes
per scenario).

=== Solution — System Property + Fakes

[source,kotlin]
----
// DownloadMusicTask.kt
companion object {
    const val MOCK_PROPERTY = "newpipe.test.mock"
}

private fun newInfoProvider(): VideoInfoProvider =
    if (System.getProperty(MOCK_PROPERTY) == "true") FakeVideoInfoProvider()
    else YouTubeDownloader()
----

[source,kotlin]
----
// TestWorld.kt — flag is passed automatically to GradleRunner
.withArguments(tasks + "--stacktrace" + "-D${DownloadMusicTask.MOCK_PROPERTY}=true")
----

Fakes return fixed data instantly — no network calls, no FFmpeg. Tests verify
folder structure, file naming, duplicate skipping and error handling in under
a second per scenario.

`FakeVideoInfoProvider.getBestAvailableBitrateKbps()` always returns `0`,
which disables the quality upgrade check in `alreadyDownloaded()` —
intentional, as upgrade logic depends on real stream data.

=== Running Tests

[source,bash]
----
# Cucumber tests only
./gradlew cucumberTest

# All tests
./gradlew check
----

== Dependencies

[cols="2,1,2", options="header"]
|===
| Dependency | Version | Role

| `com.github.TeamNewPipe:NewPipeExtractor`
| v0.26.3
| YouTube extraction (metadata + audio streams)

| `com.squareup.okhttp3:okhttp`
| 5.3.2
| HTTP client (shared between extraction and download)

| `net.jthink:jaudiotagger`
| 3.0.1
| ID3 tag and cover art writing

| `com.fasterxml.jackson.dataformat:jackson-dataformat-yaml`
| 2.21.x
| `musics.yml` parsing

| `org.jetbrains.kotlinx:kotlinx-coroutines-core`
| 1.10.2
| Concurrency (async/await, Semaphore, Dispatchers.IO)
|===

[NOTE]
====
NewPipe Extractor is resolved via https://jitpack.io[JitPack] at version
`v0.26.3` (latest stable tag, June 2026). This dependency mechanism is
accepted by the Gradle Plugin Portal but not by Maven Central.
====

== Build & Publishing

=== Local Build

[source,bash]
----
cd newpipe-plugin
./gradlew build
----

=== Publish Locally (for consumer testing)

[source,bash]
----
./gradlew publishToMavenLocal
----

=== Publish to Gradle Plugin Portal

The plugin is published on the https://plugins.gradle.org[Gradle Plugin Portal]
under the account `cccp-education`.

[source,bash]
----
./gradlew publishPlugins
----

Credentials are configured via `gradle.publish.key` and `gradle.publish.secret`
in `~/.gradle/gradle.properties` or via `GRADLE_PUBLISH_KEY` /
`GRADLE_PUBLISH_SECRET` environment variables.

[NOTE]
====
This plugin cannot be published to Maven Central because it depends on
NewPipe Extractor via JitPack (`com.github.TeamNewPipe:NewPipeExtractor`),
which is rejected by Maven Central's coordinate policy. The Gradle Plugin
Portal accepts JitPack dependencies.
====

== Known Issues

=== YouTube Throttling

YouTube throttles unofficial clients via the `n` parameter in the stream URL.
If downloads are abnormally slow on a fast connection, bumping the dependency
to the latest `dev` commit of NewPipe Extractor is the first step.

=== Mix Radio (list=RD...)

YouTube Mix Radio playlists are dynamically generated and cannot be paginated
beyond the first page. Full enumeration is not possible without the official
YouTube API.

== Roadmap

* [x] FFmpeg via Docker support (zero system dependency)
* [x] Configure the Docker image via the DSL (`ffmpegDockerImage`)
* [x] Force Docker even when FFmpeg is installed locally (`forceDocker`)
* [x] Prioritised audio stream selection (320k → 256k → 192k → 160k → fallback)
* [x] Automatic quality upgrade detection and re-download
* [x] Safe upgrade with `.old` backup — restore on failure
* [x] Two-pass ID3 tag writing with minimal fallback
* [ ] Integration tests tagged `@integration` (real downloads, nightly CI)
* [ ] Download report at end of task (JSON/HTML summary)